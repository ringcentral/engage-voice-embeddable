...
import type { LoginTypes } from '@ringcentral-integration/engage-voice-widgets/enums';
import {
  agentSessionEvents,
  dialoutStatuses,
  dropDownOptions,
  ...
  messageTypes,
  tabManagerEvents,
} from '@ringcentral-integration/engage-voice-widgets/enums';
import type { LoginType } from '@ringcentral-integration/engage-voice-widgets/interfaces/EvAgentSessionUI.interface';
import type {
  EvAgentConfig,
  EvAvailableSkillProfile,
  EvConfigureAgentOptions,
} from '@ringcentral-integration/engage-voice-widgets/lib/EvClient';
import { evStatus } from '@ringcentral-integration/engage-voice-widgets/lib/EvClient/enums';
import { TabLife } from '@ringcentral-integration/engage-voice-widgets/lib/tabLife';
import { trackEvents } from '@ringcentral-integration/engage-voice-widgets/lib/trackEvents';
import type { AgentSession, Deps, FormGroup } from '@ringcentral-integration/engage-voice-widgets/modules/EvAgentSession/EvAgentSession.interface';
import i18n from '@ringcentral-integration/engage-voice-widgets/modules/EvAgentSession/i18n';
import { tabManagerEnabled } from '@ringcentral-integration/engage-voice-widgets/modules/EvAgentSession/tabManagerEnabled.decorator';
...
class EvAgentSession extends RcModuleV2<Deps> implements AgentSession {
  ...
  @storage
  @state
  formGroup: NewFormGroup = DEFAULT_FORM_GROUP;
  ...
  get defaultDialGroupId() {
    const { agentConfig } = this._deps.evAuth;
  ...
  @action
  setFormGroup(data: NewFormGroup) {
  ...
  private async _newMainTabReConfig() {
    ...
    if (
      // !this.isReconnected && // ! that is not necessary
      this._deps.evAuth.connected &&
      this.configSuccess &&
      this.isMainTab
    ) {
  ...
  private _checkFieldsResult(formGroup: NewFormGroup): EvConfigureAgentOptions {
